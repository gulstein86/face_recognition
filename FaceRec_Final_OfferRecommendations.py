...
# Read offers dataset
df = pd.read_csv('D:\\FaceRecognition\\Offers.csv')
# Define variables for various paths
...
while True:
    ...
    for (top, right, bottom, left), name in zip(face_locations, face_names):
        ...
        if len(df[df['Name'] == name])>0 and i ==0:
            
            engine.say("Welcome " + name + ", Please check these offers, specially designed for you!")
            engine.setProperty('rate',120)  #120 words per minute
            engine.setProperty('volume',0.9) 
            engine.runAndWait()
           
            easygui.msgbox( df[df['Name'] == name], title="Offers")
            process_this_frame =4
            break 
            #print(df.Offers[df['Name'] == name])
            #time.sleep(3)
